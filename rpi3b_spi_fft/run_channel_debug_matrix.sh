...
usage() {
  cat <<'EOF'
Run a small matrix of passive channel-debug replays for the FPGA SPI FFT stream.

The script stores:
- one shared raw capture of the live SPI payload,
...
Options:
  -D, --device DEV             SPI device passed to analyzer (default: auto)
  -r, --rate HZ                Sample rate (default: 48000)
      ...
      --window-ready-line N    Optional GPIO input line captured in debug logs
      --spi-max-speed-hz HZ    SPI master clock rate (default: 8000000)
      --spi-mode N             SPI mode used by the Raspberry Pi master (default: 0)
      --bfpexp-hold-frames N   BFPEXP tagged pairs per SPI transaction (default: 1)
...
Examples:
  ...
  ./run_channel_debug_matrix.sh --device /dev/spidev0.0 --window-ready-line 23
  ./run_channel_debug_matrix.sh --output-dir debug_runs/pi_after_spi_fix
EOF
}
...

{
  ...
  echo "window_ready_line=${WINDOW_READY_LINE:-none}"
  echo "spi_max_speed_hz=${SPI_MAX_SPEED_HZ}"
  echo "spi_mode=${SPI_MODE}"
  echo "bfpexp_hold_frames=${BFPEXP_HOLD_FRAMES}"
...
Files:
- channel_capture.raw: one shared raw tagged SPI payload capture from the FPGA
- channel_capture.index.jsonl: chunk timestamps, pair counts, and sampled window_ready state
- all scenario JSONL logs replay the same raw capture for apples-to-apples comparison
- session_info.txt: environment, git state, spidev listing, and matrix parameters
...
3. If there is an external window_ready GPIO wire, include the line number used here.
...
COMMON_ARGS=(
  ...
  "--gpio-chip" "${GPIO_CHIP}"
  "--spi-max-speed-hz" "${SPI_MAX_SPEED_HZ}"
  "--spi-mode" "${SPI_MODE}"
  "--bfpexp-hold-frames" "${BFPEXP_HOLD_FRAMES}"
...
if [[ -n "${WINDOW_READY_LINE}" ]]; then
  COMMON_ARGS+=("--window-ready-line" "${WINDOW_READY_LINE}")
fi
