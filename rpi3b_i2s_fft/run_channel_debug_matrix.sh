...
- channel_capture.index.jsonl: chunk timestamps, pair counts, sampled BFPEXP flag state, and helper-side ALSA telemetry
...
printf '%s\n' \
  "scenario	status	log_file	duration_seconds	total_pairs	idle	bfpexp	fft	tag_mismatch	packet_index_mismatch	other	max_fft_run	reserved_nonzero_words	flag_high_chunks	flag_low_chunks	flag_unknown_chunks	capture_helper_events	capture_xruns	capture_recoveries	capture_partial_reads	capture_queue_high_water	capture_full_waits	capture_rate_adjusted	interrupted" \
  > "${SUMMARY_PATH}"
...
capture = summary.get("capture_diagnostics", {})
...
row = [
    ...
    str(capture.get("helper_event_count", "")),
    str(capture.get("xruns", "")),
    str(capture.get("recoveries", "")),
    str(capture.get("partial_reads", "")),
    str(capture.get("queue_high_water_slots", "")),
    str(capture.get("queue_full_waits", "")),
    str(capture.get("rate_adjusted", "")),
...
run_case() {
  ...
  if [[ "${DRY_RUN}" -eq 1 ]]; then
    extract_summary "${scenario_name}" "skipped" "${log_path}"
    return 0
  fi
