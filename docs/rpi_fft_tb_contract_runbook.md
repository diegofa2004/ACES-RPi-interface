...
- `idle*`
- `128 x BFPEXP` com `tag=1`
- `512 x FFT` com `tag=2`

O receptor Python deve ignorar o `idle` inicial e so aceitar um novo frame
de FFT depois de observar o preambulo completo de `BFPEXP`.
...
4. Pressione `Enter` no analisador para salvar o evento de referencia.
5. Confira se o `fft_latest.png` mostra o pico esperado perto de `953.67 Hz`.
